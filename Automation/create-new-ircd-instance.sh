...
OLD_ID=$(wc -w <<< $EX_IRCD)
NEW_ID=$((OLD_ID + 1))
IP_VM=$(openstack server list -c Networks -c Name --format value | grep -Po 'IRC-Server-$NEW_ID')
...
IP_VM=$(openstack server list -c Networks -c Name --format value | grep -Po '10\.11\.12\.$NEW_ID')
openstack floating ip create --floating-ip-address $FLOATIP_NEWIRCD --project $P_PROJECTN public
openstack server add floating ip --fixed-ip-address 10.11.12.2 IRC-Server-$NEW_ID $FLOATIP_NEWIRCD
